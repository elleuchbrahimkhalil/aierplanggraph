...
def _load_swagger_generated_endpoints() -> List[Dict[str, Any]]:
        ...
        if fallback_path.exists():
            ...
            for path, methods in payload.get("paths", {}).items():
                if isinstance(methods, dict):
                    swagger_paths[path] = [m.upper() for m in methods.keys()]
    ...
    for path, methods in swagger_paths.items():
        # Keep only GET for question-answer retrieval flow.
        if "GET" not in methods:
            continue
        keywords = _path_to_keywords(path)
        generated.append(
            {
                "id": _path_to_generated_id(path, "GET"),
                "method": "GET",
                "url": path,
                "intent": "GET",
                "keywords": keywords,
                "routeParameters": re.findall(r"\{([^{}]+)\}", path),
                "queryParameters": [],
                "requiredParameters": re.findall(r"\{([^{}]+)\}", path),
                "responseFormat": "Object",
                "description": f"Generated from Swagger: {path}",
                "examples": [],
                "role": _infer_domain_from_path(path),
            }
        )
...
def retrieve_candidate_endpoints(state: AssistantState) -> AssistantState:
    ...
    for ep in endpoints:
        # Keep generated Swagger endpoints focused on the inferred domain.
        ...
        role_bonus = 2 if ep.get("role") == domain else 0
        score = overlap + intent_bonus + role_bonus
        if score > 0:
...
def call_webapi(state: AssistantState) -> AssistantState:
    ...
    override_path = overrides.get(str(selected.get("id", "")))
    if override_path:
