...
def _is_client_variable_identification_question(question: str) -> bool:
    ...
    # Strict mode: only treat as metadata/schema identification when explicit schema intent exists.
    # Normal data questions like "afficher email et telephone des clients" should not match this.
    return has_client_context and has_variable_intent
...
def _select_best_endpoints(
    ...
    if use_ollama:
        system_prompt = (
            "You are a technical API Router for an ERP system. Your goal is to map a user question and its extracted data "
            "to the most appropriate WebAPI endpoints. "
            "You will receive a list of candidates and a set of parameters already extracted by another model. "
            "Logic:\n"
            "1. Match extracted parameter keys (like 'cod_art', 'id', 'num_be') to the 'queryParameters' or 'requiredParameters' of candidates.\n"
            "2. Prioritize business-oriented endpoints (GetAll, OData, list) over technical or test routes.\n"
            "3. If multiple endpoints seem relevant, pick the one that covers the most requested columns.\n"
            "Return strict JSON with fields: 'endpoint_ids' (array) and 'logical_reasoning' (brief string).\n"
        ...
        user_prompt = (
            f"Question: {question}\n"
            f"Intent: {intent}\n"
            f"Parameters already extracted from message: {json.dumps(router_params, ensure_ascii=False)}\n"
            ...
            "Select the most technicaly accurate endpoint_ids from the candidates based on the extracted parameters."
        )
        try:
...
def _build_endpoint_url(url_template: str, params: Dict[str, Any]) -> str:
    ...
    for route_key in route_params:
        ...
        if value is not None:
            rendered = rendered.replace(f"{{{route_key}}}", quote(str(value)))
    return rendered
