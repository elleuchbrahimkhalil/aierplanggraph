...
def call_webapi(state: AssistantState) -> AssistantState:
    ...
    for endpoint in selected_endpoints:
        ...
        # Special case: /api/Client/GetAllClients is metadata-only, returns schema definitions
        if "/api/client/getallclients" in endpoint.get("url", "").lower():
            mapping_db = _load_dataset_mapping_db()
            client_table = next((t for t in mapping_db.get("tables", []) if t.get("name") == "client"), None)
            if client_table:
                field_defs = [
                    {"name": col.get("name"), "type": col.get("type"), "description": col.get("description")}
                    for col in client_table.get("columns", [])
                ]
                endpoint_payload = {
                    "endpoint": endpoint.get("id"),
                    "url": endpoint.get("url"),
                    "resolvedUrl": endpoint.get("url"),
                    "fullUrl": None,
                    "query": query_params,
                    "statusCode": 200,
                    "dataCount": len(field_defs),
                    "error": None,
                    "raw": {"fields": field_defs, "description": client_table.get("description")},
                }
                for field_def in field_defs:
                    merged_data.append({
                        "endpoint": endpoint.get("id"),
                        "sourceUrl": endpoint.get("url"),
                        "record": field_def,
                    })
                call_results.append(endpoint_payload)
                continue
            else:
                errors.append(f"Client table definition not found in dataset_mapping_db for {endpoint.get('id')}")
                call_results.append({
                    "endpoint": endpoint.get("id"),
                    "url": endpoint.get("url"),
                    "resolvedUrl": endpoint.get("url"),
                    ...
                    "statusCode": None,
                    "dataCount": 0,
                    "error": "Client table definition not found",
                    "raw": None,
                })
                continue
