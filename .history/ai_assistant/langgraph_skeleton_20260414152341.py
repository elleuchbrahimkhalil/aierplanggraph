...
def _compute_endpoint_score(
    ...
    generic_client_context = _contains_any(question, ["client", "clients"]) and not bl_context
...
def evidence_filter(state: AssistantState) -> AssistantState:
    ...
    requested_fields = _extract_requested_fields(state.get("question", ""))
    filtered = records[:20]
    ...
    if requested_fields:
        ...
        for item in filtered:
            if isinstance(item, dict) and "record" in item:
                projected_record = _project_record_fields(item.get("record"), requested_fields)
