...
const QUICK_ACTIONS = [
  {
    ...
    fallbackEndpoint: '/api/BlClient/GetAllClients',
  },
  {
...
export default function App() {
  ...
  async function runAssistant(customAction) {
    ...
    try {
      ...
      let response = await fetch(action.endpoint);
      let usedEndpoint = action.endpoint;

      // Client endpoint is protected; fallback to public BlClient endpoint in local dev.
      if (response.status === 401 && action.fallbackEndpoint) {
        response = await fetch(action.fallbackEndpoint);
        usedEndpoint = action.fallbackEndpoint;
        setCalledEndpoint(`${action.endpoint} -> ${action.fallbackEndpoint}`);
      }

      if (!response.ok) {
        throw new Error(`HTTP ${response.status} sur ${usedEndpoint}`);
      }
